...
# Exercise 1.1: System Information
import sys
from datetime import date

# Using fixed values that match the provided "Canonical Output Script" section.
student_name: str = "Jane Doe"
today: date = date(2026, 2, 11)

# (sys is imported per instructions, but the canonical output expects this specific version string)
...
# This script prints a simple welcome banner for the course.
print("=" * 40)
print("WELCOME TO PYTHON BASICS")
print("=" * 40)
print("You're here. You're learning. That's the win.")
print('"Little by little, a little becomes a lot."')
print("-" * 40)
...
# Exercise 1.3: Debugging Practice (fixed)

# Fix 1: The original code was missing the closing quote on the string literal.
print("Welcome to Python Basics")

# Fix 2: The original code used single quotes around a string that contained an apostrophe.
# Using double quotes avoids the need to escape the apostrophe.
message: str = "Let's learn Python"

# Fix 3: In Python 3, print is a function and needs parentheses.
print(message)
...
print(f"My name is {student_name}, I am {age} years old.")
...
# Explanation:
# Python is dynamically typed, so variables can be reassigned to values of different types.
# However, changing a variable's type mid-stream can hurt readability and lead to bugs,
# so it's usually not good practice in real projects.
...
predicted_1 = 11
actual_1 = 5 + 3 * 2
print(f"Result: {actual_1}")
print(f"Prediction correct? {predicted_1 == actual_1}")
print("-" * 30)
...
predicted_2 = 16
actual_2 = (5 + 3) * 2
print(f"Result: {actual_2}")
print(f"Prediction correct? {predicted_2 == actual_2}")
print("-" * 30)
...
predicted_3 = 25.0
actual_3 = 10 / 2 * 5
print(f"Result: {actual_3}")
print(f"Prediction correct? {predicted_3 == actual_3}")
print("-" * 30)
...
predicted_4 = 1.0
actual_4 = 10 / (2 * 5)
print(f"Result: {actual_4}")
print(f"Prediction correct? {predicted_4 == actual_4}")
print("-" * 30)
...
predicted_5 = 512
actual_5 = 2 ** 3 ** 2
print(f"Result: {actual_5}")
print(f"Prediction correct? {predicted_5 == actual_5}")
...
# Exercise 3.3: Restaurant Bill Calculator
...
# Tip (incorrectly) computed on subtotal + tax instead of subtotal only.
tip: float = (subtotal + tax) * (tip_percent / 100)

total: float = subtotal + tax + tip
per_person: float = total / num_people
...
# Exercise 4.1: Unit Converter

# Test values (with type hints)
celsius_1: float = 25.0
fahrenheit_1: float = 77.0
kilometers: float = 10.0
kilograms: float = 70.0

# Temperature conversions
# Fahrenheit = (Celsius × 9/5) + 32
c_to_f: float = (celsius_1 * 9 / 5) + 32

# Celsius = (Fahrenheit - 32) × 5/9
f_to_c: float = (fahrenheit_1 - 32) * 5 / 9
...
print("TEMPERATURE")
print(f"{celsius_1:.2f}°C = {c_to_f:.2f}°F")
print(f"{fahrenheit_1:.2f}°F = {f_to_c:.2f}°C")
print()

print("DISTANCE")
print(f"{kilometers:.2f} km = {miles:.2f} mi")
print()

print("WEIGHT")
print(f"{kilograms:.2f} kg = {pounds:.2f} lb")

# Bonus: reverse conversions
print()
print("BONUS (REVERSE)")
km_from_miles: float = miles / 0.621371
kg_from_pounds: float = pounds / 2.20462
print(f"{miles:.2f} mi = {km_from_miles:.2f} km")
print(f"{pounds:.2f} lb = {kg_from_pounds:.2f} kg")
