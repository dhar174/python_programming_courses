...
def extract_slides(html_path):
    ...
    if not sections:
        # Don't fallback to all sections if it's an index/portal file
        if Path(html_path).name == 'index.html':
            sections = []
        else:
...
def compare_directories(backup_dir, new_dir):
    ...
    for f in sorted(all_files):
        if f not in backup_files:
            ...
            lines_added = len(new_full_text)

            report_data['total_lines_changed'] += lines_added
            if len(new_slides) > 0 or lines_added > 0:
                report_data['file_details'][str(f)] = {
                    'slides_added': len(new_slides),
                    'slides_removed': 0,
                    'lines_added': lines_added,
                    'lines_removed': 0,
                    'text_diff': [f"+ {line}" for line in new_full_text[:50]]
                }
            continue
        ...
        if f not in new_files:
            ...
            lines_removed = len(old_full_text)

            report_data['total_lines_changed'] += lines_removed
            if len(old_slides) > 0 or lines_removed > 0:
                report_data['file_details'][str(f)] = {
                    'slides_added': 0,
                    'slides_removed': len(old_slides),
                    'lines_added': 0,
                    'lines_removed': lines_removed,
                    'text_diff': [f"- {line}" for line in old_full_text[:50]]
                }
            continue
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Compare slide directories and generate a report.")
    parser.add_argument("--basics-backup", default="backup_main Basics-lessons_slides", help="Path to Basics backup directory")
    parser.add_argument("--basics-new", default="Basics/lessons/slides", help="Path to new Basics slides directory")
    parser.add_argument("--advanced-backup", default="backup_main advanced slides", help="Path to Advanced backup directory")
    parser.add_argument("--advanced-new", default="Advanced/lessons/slides", help="Path to new Advanced slides directory")
    args = parser.parse_args()

    print("Comparing Basics slides...")
    basics_data = compare_directories(args.basics_backup, args.basics_new)

    print("Comparing Advanced slides...")
    advanced_data = compare_directories(args.advanced_backup, args.advanced_new)
