Instructor Runbook
Python Programming (Basic) – 48 Hours
...
Live-code 5 common mistakes and read the error message out loud.

Show how to fix each in <30 seconds.
...
Demo: build a small ‘profile card’ with variables; change values and re-run.
...
Demo: mini calculator; show / vs //; show % with even/odd check.
...
Demo: extract initials, domain from email, and last character with -1.
...
Demo: clean messy input; show before/after; show find() returning -1.
...
Demo: small converter; show ValueError if input is not numeric (save full handling for exceptions day).
...
Demo: walk through sample rubric; show how you’ll grade.
...
Demo: age gating; show two comparisons combined with and.
...
Add a second condition (e.g., has_membership).
...
Demo: take Tip Calculator and reformat output; show alignment with format spec.
...
Demo: compute word count and rebuild sentence with hyphens.
...
Demo: build shopping list; append, remove, pop; print after each operation.
...
Demo: sum numbers in a list; compute average.
...
Quick check: What does ‘for x in my_list’ give you each iteration?
...
Demo: seating chart; print rows; update one seat.
...
Demo: sample solution walkthrough at a high level (don’t give full code).
...
Demo: return-like tuple (x, y); unpack; show immutability error.
...
Demo: list of names → set; show duplicates removed.
...
Show both: sorted(unique_set) for display.
...
Demo: inventory dict; update quantities; show KeyError vs get().
...
Demo: word frequency from a sentence.
...
Demo: membership lookup in list vs set (conceptual; no benchmarking needed).
...
Demo: one station solution quickly; show expected output.
...
Demo: scaffold a menu and one action (Add).
...
Demo: show how to test with 2–3 example inputs.
...
Demo: shipping tiers; show correct boundary handling.
...
Demo: password attempts loop; show break on success.
...
Demo: multiplication table rows using nested loops (keep small).
...
Demo: read 5 numbers; compute sum, min, max.
...
Demo: menu scaffold with 3 options and a quit option.
...
Demo: safe int input function using while + isdigit().
...
Demo: implement add + list quickly; show testing strategy.
...
Demo: show how you’ll spot-check quickly (run, try invalid input, try missing key).
...
Demo: refactor a calculator into functions; show unit-like manual testing.

Hands-on lab (25–35 min)

Lab: Refactor Contact Manager

- Extract 3 functions: add_contact, list_contacts, search_contact.
...
Demo: show a bug caused by relying on global, then fix by passing list/dict.
...
Demo: normalize list of names; compare in-place vs new list.
...
- Add: safe_int(prompt) and format_money(value).
...
Add a third helper: menu_choice(prompt, options).
...
Demo: Contact class with name/phone; display() method.
...
Demo: search_contact() returning an object; then update it.
...
Demo: Contact.update_phone checks length/digits; if invalid, returns False or raises ValueError.
...
Demo: run through a checklist of required features.
...
Demo: write a shopping list to file; then read and print.
...
Demo: dump dict/list to data.json; load; print.

If using objects: show to_dict().
...
Demo: wrap numeric input and file loading; show friendly error messages.
...
Demo: scaffold menu + load data + one action + save.
...
Demo: implement one feature end-to-end with save/load.
...
Demo: show a ‘good demo’ flow and how to handle a mistake live.
...
Demo: quick ‘read this code and predict output’ exercise.
...
| FileNotFoundError | Wrong path; print current working directory; use pathlib and create data/ folder. |
