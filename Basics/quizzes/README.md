...
These JSON files are **generated by learners when they export their answers from the quiz HTML page**. In addition, the repository may include committed sample export files as grader/reference fixtures (for example, Day 2 and Day 3 sample answers exports), while real learner submissions still use the same filename and JSON shape.
