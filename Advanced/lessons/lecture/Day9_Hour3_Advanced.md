# Day 9, Hour 3: Serialization + validation (manual, consistent)

**Python Programming Advanced - Session 9**
**Runbook alignment:** Session 9, Hour 35
...
- Keep the authoritative runbook open to Session 9, Hour 35; this script expands that hour into a near-verbatim delivery guide.
...
- Serialization means converting Python objects to plain JSON-ready dictionaries.
- Deserialization means reading client payloads and turning them into validated data.
- Manual validation is acceptable in this course when it is centralized and consistent.
- The contract is part of the product; changing field names randomly breaks clients.
...
1. Extract to_dict and parse_record_payload helpers.
2. Show a happy payload becomes a domain object or command dict.
3. Show missing field, wrong type, and unknown field sad paths.
4. Update one route to become shorter after moving validation out.
...
Clean API contracts: create api/serializers.py or a similarly named module, route every payload through one parser, and manually test three negative cases: missing field, bad type, and missing id.
...
- Routes become shorter because parsing and serialization are reusable.
...
- Duplicating the same validation in GUI, API, and repository.
- Accepting unknown fields silently when they hide typos.
- Forgetting that JSON has no datetime object; convert dates to strings.
...
Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **Serialization and validation**. The concrete artifact for this hour is manual serializer and validator helpers with one consistent external JSON shape. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the data contract between internal domain objects and API payloads. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline command is POST payloads that include valid values, missing fields, extra fields, and incorrect types. A successful run should prove this happy path: a domain record is converted into stable JSON with the intended field names and value formats. A responsible implementation should also prove this sad path: missing required fields and invalid values return `validation_error` without a stack trace. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to write `record_to_dict`, write `parse_record_payload`, then compare internal names to external JSON names. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run POST payloads that include valid values, missing fields, extra fields, and incorrect types before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove a domain record is converted into stable JSON with the intended field names and value formats. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger missing required fields and invalid values return `validation_error` without a stack trace deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: a domain record is converted into stable JSON with the intended field names and value formats."
- "Show me the sad-path evidence: missing required fields and invalid values return `validation_error` without a stack trace."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: include an `updated_at` field formatted as ISO 8601 and document whether clients may send it. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Which fields are part of the public API, and which are internal implementation details?
...
Name the common risk explicitly: returning `record.__dict__` and accidentally exposing private fields or unstable implementation details. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
