...
**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline is three write requests: missing key, wrong key, and correct key. A successful run should prove this happy path: a request with the configured key reaches the service and returns the normal success payload. A responsible implementation should also prove this sad path: missing or invalid keys return a documented 401 or 403 JSON error; missing server config returns a clear 500 during development. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
