# Day 10, Hour 1: API security basics: API key gate + safe storage

**Python Programming Advanced - Session 10**
**Runbook alignment:** Session 10, Hour 37
...
- Keep the authoritative runbook open to Session 10, Hour 37; this script expands that hour into a near-verbatim delivery guide.
...
- This is not a full authentication system; it is a practical boundary for a course project.
- Secrets do not belong in Git, screenshots, or hard-coded constants.
- GET can remain open while POST, PUT, and DELETE require X-API-Key.
- 401 means credentials are missing or unacceptable; 403 means authenticated but not allowed. For a simple key gate, choose and document one convention.
...
1. Read TRACKER_API_KEY from os.environ.
2. Create require_api_key() and call it in write routes or a decorator.
3. Show POST without header fails, wrong header fails, correct header succeeds.
4. Point out that JSON error contracts remain unchanged.
...
Protect write endpoints: set TRACKER_API_KEY in the shell or run configuration, require X-API-Key for POST/PUT/DELETE, and verify missing, wrong, and correct credentials.
...
- Missing/wrong keys return JSON with the shared error contract.
...
- Hard-coding the key in app.py.
- Protecting POST but forgetting PUT or DELETE.
- Returning a default Flask HTML error.
- Printing secrets in logs or demo screenshots.
...
- Add a pure function that validates a header dict for easy testing.
- Add a toy delay after repeated failures, clearly labeled non-production.
- Document environment variables in README without revealing real values.
...
- What status code fits missing or invalid credentials in this simple API-key design?
...
Before closing this hour, explicitly connect the API key gate to the later client timeout conversation: protected write calls should fail quickly and clearly when credentials are missing, and client code should use a timeout so a security or network problem does not hang the learner interface.

## Additional Instructor-Ready Expansion

Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **API key gate and safe secret handling**. The concrete artifact for this hour is write routes protected by `X-API-Key` with the expected key read from the environment. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the security boundary at the API edge without turning the capstone into a full auth system. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline command is three write requests: missing key, wrong key, and correct key. A successful run should prove this happy path: a request with the configured key reaches the service and returns the normal success payload. A responsible implementation should also prove this sad path: missing or invalid keys return a documented 401 or 403 JSON error; missing server config returns a clear 500 during development. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to read `TRACKER_API_KEY`, add a decorator or helper, apply it only to modifying routes, then repeat the same request with different headers. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run three write requests: missing key, wrong key, and correct key before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove a request with the configured key reaches the service and returns the normal success payload. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger missing or invalid keys return a documented 401 or 403 JSON error; missing server config returns a clear 500 during development deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: a request with the configured key reaches the service and returns the normal success payload."
- "Show me the sad-path evidence: missing or invalid keys return a documented 401 or 403 JSON error; missing server config returns a clear 500 during development."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add a README section showing how to set the environment variable using a placeholder value only. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Can a reviewer prove security behavior without seeing or knowing the real key?
...
Name the common risk explicitly: committing secrets in source code, screenshots, notebooks, README examples, or shell history copied into documentation. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
