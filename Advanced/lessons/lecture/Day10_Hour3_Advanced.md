# Day 10, Hour 3: Integration choice: GUI talks to API or parallel UI/API

**Python Programming Advanced - Session 10**
**Runbook alignment:** Session 10, Hour 39
...
- Explain the tradeoff between GUI->API and parallel surfaces over one service layer.
...
- Keep the authoritative runbook open to Session 10, Hour 39; this script expands that hour into a near-verbatim delivery guide.
...
- Option A: GUI calls API, so the API becomes the single gateway.
- Option B: GUI uses service/repository locally while API uses the same service design.
- The danger is not choosing either; it is accidentally maintaining two incompatible apps.
- Field names, validation rules, and database schema must stay aligned.
...
1. Draw the two architecture options on the board.
2. Demo one create operation and show it appears in the other surface.
3. Show a sad path: validation failure from GUI or API maps to a friendly message.
4. Ask learners to state which option they choose and why.
...
Integration sprint: choose A) modify GUI to call the API for CRUD, or B) keep GUI local and ensure API uses the same service and SQLite repository. Minimum proof: both surfaces operate on the same underlying data.
...
- Two schemas: name in GUI, title in API, item_name in database.
- GUI does not refresh after an API call.
- Running API and GUI against different database files.
...
- Add a GUI connectivity indicator.
- Add a settings field for API base URL.
- Add a small sync button that reloads data from the API.
...
- What is the biggest tradeoff of your chosen architecture: simplicity, testability, latency, or deployment?
...
Name the integration contract out loud: if the GUI talks to the API, it must send the same JSON shape, include the API key for write operations, and rely on client functions that set a timeout. If the GUI remains local, the API key and timeout concerns still belong in the parallel API/client surface.

## Additional Instructor-Ready Expansion

Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **GUI/API integration choice**. The concrete artifact for this hour is a documented decision: GUI talks through the API client, or GUI and API run as parallel interfaces over shared services. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to draw both architectures, implement one small callback or adapter, and explain why that choice fits the capstone constraints. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run one visible GUI action or equivalent script that proves the chosen path works end-to-end before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove the same record behavior is available from the chosen interface without conflicting persistence rules. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger server unavailable, stale data, or validation errors are shown to the user without hiding the cause deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: the same record behavior is available from the chosen interface without conflicting persistence rules."
- "Show me the sad-path evidence: server unavailable, stale data, or validation errors are shown to the user without hiding the cause."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add a settings object that switches between local-service mode and API-client mode. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Where is the business rule written exactly once after the integration choice?
...
Name the common risk explicitly: having the GUI, API, and service each implement a different version of validation or update semantics. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
