...
**Runbook alignment:** Session 12, Hour 46
...
- Measure test coverage and interpret it responsibly.
...
- Keep the authoritative runbook open to Session 12, Hour 46; this script expands that hour into a near-verbatim delivery guide.
...
- Coverage tells us what code ran; it does not prove correctness.
- Use bug history and risk to choose tests, not only uncovered line numbers.
- A lightweight integration test can use a temporary SQLite database without launching the whole UI.
- Reasonable confidence beats chasing 100% coverage.
...
1. Run pytest with coverage if available.
2. Inspect the lowest or riskiest uncovered areas.
3. Add tests for not-found, invalid status, and persistence across repository instances.
4. Discuss why route tests should stay minimal in this course.
...
Harden with tests: run a coverage report if available, add two tests for high-value missing branches, and add one integration test covering service plus SQLite repository.
...
- Edge cases include missing record and invalid input.
- Learner can explain what coverage does and does not guarantee.
...
- Chasing 100% coverage with low-value tests.
- Testing Flask routes heavily when pure functions would be simpler.
- Ignoring nondeterministic behavior such as dates and random ids.
...
- Add a pure-function test for API key protection logic.
- Generate an HTML coverage report if allowed.
- Add a smoke test for create_app with Flask test_client().
...
- Why can high coverage still miss bugs?
...
Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **Coverage, edge cases, and lightweight integration testing**. The concrete artifact for this hour is targeted tests for edge cases plus one integration test that crosses service, repository, and API/client boundary. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to read a coverage report, choose one untested risk, write a test that fails for the right reason, then make it pass. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run `pytest -q --cov` or the course-approved coverage command plus one focused integration test file before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove coverage increases because meaningful behavior is tested, not because trivial lines are exercised. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger edge cases such as not found, duplicate values, empty data, and bad JSON are covered by readable tests deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: coverage increases because meaningful behavior is tested, not because trivial lines are exercised."
- "Show me the sad-path evidence: edge cases such as not found, duplicate values, empty data, and bad JSON are covered by readable tests."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add Flask test-client checks for one authenticated write route and one validation error route. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Which untested behavior would most embarrass the final demo if it failed?
...
Name the common risk explicitly: chasing a coverage percentage while ignoring the behaviors most likely to break the capstone demo. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
