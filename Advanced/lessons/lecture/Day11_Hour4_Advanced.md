# Day 11, Hour 4: Integrate analytics into the capstone report feature

**Python Programming Advanced - Session 11**
**Runbook alignment:** Session 11, Hour 44
...
- Keep the authoritative runbook open to Session 11, Hour 44; this script expands that hour into a near-verbatim delivery guide.
...
- Keep a reporting failure checklist visible: missing CSV, missing required columns, empty cleaned data, unwritable reports directory, and how each failure should be explained to a reviewer.
...
- The pipeline is export -> analyze -> chart -> save artifacts.
- Use pathlib and create output directories in code.
- Avoid manual notebook-only steps for the final deliverable unless clearly documented.
...
2. Call repository export or read existing CSV.
3. Generate summary.csv and at least one PNG.
4. Run python -m reports.generate_report twice to prove repeatability.
...
Report integration: add a command or GUI/API action that generates CSV export, summary table, and at least one plot saved to reports/. Document exact run steps in README.
...
- Paths are relative/project-safe and created as needed.
...
- Report depends on manual copy/paste steps.
- Charts overwrite unexpectedly without a naming plan.
- README says run a command that no longer exists.
...
- Add a report date stamp to filenames.
- Expose /reports/latest in the Flask API as metadata only.
- Add a GUI button that calls the report generation function.
...
- What makes a report feature repeatable rather than just a one-time analysis?
...
Confirm the report feature uses pandas for tables and matplotlib for at least one saved chart. The finished capstone report should leave behind files in reports/ that another person can inspect without rerunning the application.

## Additional Instructor-Ready Expansion

Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **Analytics integration into capstone report**. The concrete artifact for this hour is a report command or function that generates summary text, CSV, and chart artifacts together. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the delivery pipeline connecting persisted data, pandas analysis, matplotlib output, and user-facing report documentation. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline command is run one command that creates or refreshes all report artifacts from current data. A successful run should prove this happy path: the report folder contains regenerated files with timestamps or clear names and the README explains how to produce them. A responsible implementation should also prove this sad path: no data, partial data, or missing optional dependencies are handled with clear messages and no corrupted report state. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to wire cleaning, summary, chart, and interpretation into one orchestrating function with small helpers underneath. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run run one command that creates or refreshes all report artifacts from current data before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove the report folder contains regenerated files with timestamps or clear names and the README explains how to produce them. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger no data, partial data, or missing optional dependencies are handled with clear messages and no corrupted report state deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: the report folder contains regenerated files with timestamps or clear names and the README explains how to produce them."
- "Show me the sad-path evidence: no data, partial data, or missing optional dependencies are handled with clear messages and no corrupted report state."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add a `--output-dir` argument or report metadata file describing source data and generation time. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Can the report be regenerated after deleting the old artifacts?
...
Name the common risk explicitly: copy-pasting notebook cells into a script without creating a repeatable pipeline. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
