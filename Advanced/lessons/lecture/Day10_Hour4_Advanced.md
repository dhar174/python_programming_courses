...
**Runbook alignment:** Session 10, Hour 40
...
- Keep the authoritative runbook open to Session 10, Hour 40; this script expands that hour into a near-verbatim delivery guide.
...
- We grade correctness, contract consistency, safe errors, and structure.
- Show the happy path first, then deliberately prove sad paths.
- If something fails, narrate the debugging plan calmly rather than hiding it.
...
1. Model a five-minute demo: health, list, create, update, delete, auth failure.
2. Use a clean sequence of curl commands or a client script.
3. Show where the route, service, and repository code live.
4. End with one maintainability improvement the student made.
...
# 1. GET /health -> 200
# 2. GET /records -> 200 with data list
# 3. POST /records without X-API-Key -> 401 JSON error
# 4. POST /records with key -> 201 created record
# 5. PUT /records/<id> -> 200 updated record
# 6. DELETE /records/<id> -> 204 or documented success response
# 7. GET /records/<id> -> 404 JSON error after delete
...
- API works reliably through the demo sequence.
- Persistence is proven by restarting or re-querying the database.
...
- Returning HTML errors during sad-path demos.
...
- Add a Postman/curl collection or client script for reviewers.
...
During checkpoint review, verify that the API key protects write operations and that any Python client or demo script uses a timeout. These two details prove the API milestone handles both security basics and realistic network behavior.

## Additional Instructor-Ready Expansion

Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **API checkpoint demo**. The concrete artifact for this hour is a concise milestone demonstration showing health, CRUD, API key behavior, client access, and known gaps. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the delivery and review boundary for the API portion of the capstone. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline command is a rehearsed script of commands or requests that can be repeated by a reviewer. A successful run should prove this happy path: the demo proves one full create/read/update/delete flow and one client call. A responsible implementation should also prove this sad path: the demo intentionally shows at least one authentication or validation failure with the expected JSON error. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to start from clean instructions, run the server, execute request sequence, show code boundaries, and log remaining risks. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run a rehearsed script of commands or requests that can be repeated by a reviewer before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove the demo proves one full create/read/update/delete flow and one client call. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger the demo intentionally shows at least one authentication or validation failure with the expected JSON error deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: the demo proves one full create/read/update/delete flow and one client call."
- "Show me the sad-path evidence: the demo intentionally shows at least one authentication or validation failure with the expected JSON error."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add a short API checklist to README with endpoint, method, auth requirement, and expected status codes. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Could another learner run your checkpoint from the instructions without watching you do it?
...
Name the common risk explicitly: performing an improvised demo that hides setup assumptions and cannot be reproduced after class. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
