# Day 11, Hour 2: Visualization with matplotlib: charts you can ship

**Python Programming Advanced - Session 11**
**Runbook alignment:** Session 11, Hour 42
...
- Save figures to files for a repeatable report workflow.
...
- Keep the authoritative runbook open to Session 11, Hour 42; this script expands that hour into a near-verbatim delivery guide.
...
- Keep an analytics artifact checklist visible: saved file path, chart title, labeled axes, readable categories, and a short sentence explaining what the chart answers.
...
- A chart is a communication tool, not decoration.
- Readable labels and titles matter more than fancy styling.
- Always save and close figures in scripts to avoid overlapping plots.
- Use the same cleaned dataset from Hour 41 so the pipeline stays coherent.
...
1. Load the cleaned or summarized data.
2. Create a bar chart of count by status/category.
3. Save reports/status_counts.png.
4. Show an unlabeled chart briefly, then improve it with title, axes, and rotation.
...
Generate charts: create one or two plots from the dataset, save them into reports/, and verify that each chart has a title, labeled axes, and readable text.
...
- At least one PNG chart is saved under reports/.
- Chart answers a clear question about the tracker data.
- Axes and titles are readable.
...
- Forgetting plt.close(), causing overlapping charts.
- Unlabeled axes or ambiguous titles.
- Using a pie chart where categories are too many.
- Saving to a directory that may not exist.
...
- Add a line chart for records over time.
- Add a histogram for numeric amount/priority/duration.
- Create a chart function that accepts a DataFrame and output path.
...
- Why should a report script save plots to files instead of only displaying them?
...
Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **Matplotlib charts that can ship**. The concrete artifact for this hour is one or more saved charts in `reports/` with readable labels and a reproducible generation command. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the visual report artifact built from cleaned capstone data. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline command is run the chart script, open or inspect the saved PNG, and confirm the file path is project-relative. A successful run should prove this happy path: the chart communicates a clear comparison or trend with titles, axis labels, and readable sizing. A responsible implementation should also prove this sad path: empty data, missing columns, and unreadable labels are handled before shipping the chart. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to create a small grouped DataFrame, choose bar or line intentionally, save with `tight_layout`, and discuss what the chart does not prove. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run run the chart script, open or inspect the saved PNG, and confirm the file path is project-relative before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove the chart communicates a clear comparison or trend with titles, axis labels, and readable sizing. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger empty data, missing columns, and unreadable labels are handled before shipping the chart deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: the chart communicates a clear comparison or trend with titles, axis labels, and readable sizing."
- "Show me the sad-path evidence: empty data, missing columns, and unreadable labels are handled before shipping the chart."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: generate both a summary CSV and matching chart from the same cleaned DataFrame. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: What decision or question should this chart help a reader answer?
...
Name the common risk explicitly: making an attractive chart that misleads because categories are unsorted, axes are unlabeled, or units are unclear. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
