# Day 9, Hour 2: CRUD endpoints for your main resource

**Python Programming Advanced - Session 9**
**Runbook alignment:** Session 9, Hour 34
...
- Keep the authoritative runbook open to Session 9, Hour 34; this script expands that hour into a near-verbatim delivery guide.
...
- Use plural nouns for collections: /records and /records/<record_id>.
- Route handlers should be thin adapters: parse input, call service, serialize output.
- 400 means the request is malformed; 404 means the resource was well-formed but not found.
- Use parameterized repository methods underneath; never build SQL from request text.
...
1. Start with GET /records against existing repository data.
2. Add POST /records and return 201 with the created record.
3. Add GET/PUT/DELETE by id and show a not-found response.
4. Use curl or requests to show a happy create and a sad missing-field request.
...
Build CRUD API: implement /records and /records/<id> endpoints backed by the existing service layer and SQLite repository. Test list, create, get by id, update, delete, bad JSON, validation failure, and missing id.
...
- All five CRUD routes respond with JSON.
...
- Using one route for every operation and branching manually.
- Treating string ids and integer ids inconsistently.
- Returning 200 for deletes that failed.
- Letting sqlite exceptions leak as HTML 500 pages.
...
- Add q= search filtering.
- Return 204 for successful delete and discuss why it has no body.
- Add a route-level smoke test for the create/list cycle.
...
Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **CRUD endpoints for the main resource**. The concrete artifact for this hour is list, create, read-by-id, update, and delete routes backed by the service layer. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the Flask route translation with business behavior still delegated to services. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline command is a sequence of curl, HTTPie, or requests calls for list, post, get, put, and delete. A successful run should prove this happy path: a created record can be retrieved by id and appears in the list response. A responsible implementation should also prove this sad path: bad JSON, validation failure, and missing ids produce 400 or 404 JSON errors. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to build one route at a time, run it, then deliberately send malformed input before moving to the next route. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Run a sequence of curl, HTTPie, or requests calls for list, post, get, put, and delete before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove a created record can be retrieved by id and appears in the list response. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger bad JSON, validation failure, and missing ids produce 400 or 404 JSON errors deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: a created record can be retrieved by id and appears in the list response."
- "Show me the sad-path evidence: bad JSON, validation failure, and missing ids produce 400 or 404 JSON errors."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add a route-level smoke test for create/list or support a simple `status` query parameter. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Can the same service method still be used by a CLI, GUI, test, or report without importing Flask?
...
Name the common risk explicitly: implementing business rules directly in route functions because the endpoint is visible and tempting. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
