# Day 10, Hour 2: Consuming your own API: Python client functions

**Python Programming Advanced - Session 10**
**Runbook alignment:** Session 10, Hour 38
...
- Keep the authoritative runbook open to Session 10, Hour 38; this script expands that hour into a near-verbatim delivery guide.
...
- Client functions should mirror API use cases, not every internal implementation detail.
- A timeout is not optional; without it the program can hang forever.
- Clients need both happy-path parsing and sad-path handling.
- A useful client hides repetitive URL construction and headers.
...
1. Create TrackerApiClient with base_url, api_key, timeout.
2. Implement list_records and create_record first.
3. Show a successful create/list cycle.
4. Stop the server or use a bad port to show timeout/connection error handling.
...
Build a client: create client/api_client.py with list_records, create_record, update_record, and delete_record. Add timeouts, handle common requests exceptions, and print or return user-friendly messages.
...
- Why should a client set a timeout by default even during local development?
...
Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **Python client for the API**. The concrete artifact for this hour is a `TrackerApiClient` that wraps HTTP details in Python methods. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to write `_headers`, `_request`, `list_records`, and `create_record` before adding update/delete. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.
...
- "Show me the happy-path evidence: client methods return useful Python data rather than raw response objects for common operations."
...
Name the common risk explicitly: forgetting timeouts and causing a GUI or terminal workflow to hang when the server is unavailable. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
