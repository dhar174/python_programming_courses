# Day 12, Hour 3: Packaging and delivery: requirements, runnable app, optional executable

**Python Programming Advanced - Session 12**
**Runbook alignment:** Session 12, Hour 47
...
- Produce a runnable deliverable with clear installation and run steps.
- Record dependencies and environment variables.
- Optionally build a wheel or executable when tooling is available.

## Instructor Prep Notes

- Confirm learners are in the correct project folder and virtual environment.
- Keep the authoritative runbook open to Session 12, Hour 47; this script expands that hour into a near-verbatim delivery guide.
...
- Keep a delivery checklist visible: clean virtual environment, `requirements.txt`, documented entry point, reproducible run command, test command, and optional build artifact.
...
- The minimum deliverable is a source release with README, requirements, entrypoint, and smoke test.
- Pin or at least record dependencies; document environment variables like TRACKER_API_KEY.
- Use pathlib-based paths so the app does not depend on your IDE working directory.
...
1. Review README quickstart requirements.
2. Create or update requirements.txt and requirements-dev.txt.
3. Run through a fresh environment checklist verbally.
4. If allowed, show python -m build or PyInstaller as optional, not required.
...
Ship your capstone: update README, requirements, optional dev requirements, .gitignore, and run instructions. Perform a fresh-run test in a clean venv or clean folder if lab policy allows.
...
- Packaging the wrong entrypoint.
- README commands work only in the instructor machine.
- Relative file paths break outside the IDE.
...
- Add Makefile or scripts/run_api.sh style helpers.
- Add a version string and changelog note.
- Build a wheel with python -m build if policy allows.
...
- What is the difference between "it runs on my machine" and "it ships"?
...
A shippable package should include enough test evidence to trust it: pytest should pass, and coverage output from the previous hour can be mentioned in the README or release notes as supporting evidence.

## Additional Instructor-Ready Expansion

Use this expansion to deepen the hour without changing the runbook mapping. The focus remains **Packaging and delivery**. The concrete artifact for this hour is requirements, run instructions, project structure cleanup, and optional executable or entry point. Keep returning to that artifact whenever discussion becomes abstract: learners should be able to point at a command, a file, a response, a chart, a test, or a README step and say, "This is the evidence that the hour worked."

### Deeper instructor narration

**[Instructor speaks:]**
"The most important habit in this hour is separating the visible surface from the rule underneath it. Today we are working at the delivery boundary that lets someone else install, configure, run, and verify the capstone. That layer matters because it is where another human or another part of the system forms expectations. If we leave the expectation implicit, the next person has to guess. If we make it explicit, the project becomes easier to test, easier to debug, and easier to explain during the final capstone review."

**[Instructor speaks:]**
"Before I run anything, I want us to predict the evidence. Our baseline is to follow README setup from a clean terminal and start the chosen app surface. A successful run should prove this happy path: dependencies install, the app starts, tests run, and generated artifacts land in documented paths. A responsible implementation should also prove this sad path: missing environment variables, absent database files, or platform path differences produce clear recovery steps. Notice that we are not adding sad paths to be negative. We are adding them because production code spends much of its life receiving imperfect input, missing configuration, stale data, or unexpected user behavior."
...
Run the demo as a sequence of small predictions rather than one long typing performance. The suggested demo arc is to audit imports, regenerate requirements, verify commands from README, and remove machine-specific assumptions. At each pause, ask the room to identify three things: the input entering this layer, the output leaving this layer, and the single responsibility that should not leak into neighboring layers. If learners cannot name those three things, slow down and draw the boundary again.

Use this checkpoint rhythm:

1. **Baseline:** Follow the README setup steps from a clean terminal and start the chosen app surface before editing. If it fails, narrate the failure honestly and recover before adding new code.
2. **First slice:** Implement only enough to prove dependencies install, the app starts, tests run, and generated artifacts land in documented paths. Do not add optional polish yet.
3. **Read the code aloud:** Point to the function or method boundary and say what it accepts and returns.
4. **Sad path:** Trigger missing environment variables, absent database files, or platform path differences produce clear recovery steps deliberately. Ask, "Is this failure understandable to the person who receives it?"
...
- "Show me the happy-path evidence: dependencies install, the app starts, tests run, and generated artifacts land in documented paths."
- "Show me the sad-path evidence: missing environment variables, absent database files, or platform path differences produce clear recovery steps."
- "Show me where this behavior would be changed if the requirement changed next week."

If a learner is ahead, direct them to this extension only after the required artifact is stable: add a console script entry point or a small launcher script for the preferred interface. Make it clear that extension work must be reversible. A good extension leaves the required path cleaner or better documented. A risky extension creates a second debugging problem right before the hour closes.

For pairs, assign roles for five-minute intervals. The driver types and runs commands. The navigator reads the contract and watches for boundary violations. Halfway through, switch roles. This keeps faster learners from taking over and gives quieter learners a defined speaking responsibility. Ask the navigator to use the review question: Could an evaluator start from the repository and reach the demo without asking you for hidden steps?
...
Name the common risk explicitly: shipping a project that works only in the original IDE because setup knowledge lives in the developer’s memory. Write it on the board as an anti-pattern, then ask learners to point to the line or step in their project that prevents it.
